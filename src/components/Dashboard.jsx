...
//   // Fetch real user exam results for recent activity
...
//         const res = await axios.get('http://localhost:5000/api/exam/user/results', {
//           headers: { Authorization: `Bearer ${token}` }
//         });
//         // Map backend results to dashboard format
//         const mapped = res.data.results.map(r => ({
//           title: r.exam?.title || 'Exam',
//           subject: r.exam?.skillName || 'Skill',
//           score: r.score,
//           level: r.exam?.difficulty || 'N/A',
...
//   // recentActivity is now fetched from backend
...
export default function Dashboard({ user, onLogout, onNavigate }) {
  ...
  // Fetch real exam results (MongoDB)
  useEffect(() => {
    async function fetchRecentActivity() {
      try {
        ...
        const res = await axios.get(
          "http://localhost:5000/api/exam/user/results",
          { headers: { Authorization: `Bearer ${token}` } }
        );

        const formatted = res.data.results.map((r) => ({
          title: r.exam?.title || "Exam",
          subtitle: r.exam?.skillName || r.exam?.category || "Skill",
          badge: r.exam?.difficulty || "N/A",
          score: `${r.score}%`,
        ...
        console.log("Recent Activity Fetch Error:", error);
        setRecentActivity([]);
      }
    }
  ...
  const recommendedSkills = [
    {
      ...
      level: "Intermediate",
      progress: 75,
      description: "Regression, classification, clustering, and performance metrics.",
      tags: ["Core", "Python", "Data Science"],
      difficulty: "Medium",
      image: "https://images.pexels.com/photos/8386440/pexels-photo-8386440.jpeg?auto=compress&w=400",
      estimatedTime: "2 hours",
    },
    {
      title: "Natural Language Processing (NLP)",
      level: "Expert",
      progress: 45,
      description: "Transformers, embeddings, sentiment analysis, seq2seq tasks.",
      tags: ["Advanced", "Transformers", "Python"],
      difficulty: "Hard",
      image: "https://images.pexels.com/photos/8386434/pexels-photo-8386434.jpeg?auto=compress&w=400",
      estimatedTime: "3 hours",
    },
    {
      title: "Computer Vision & GANs",
      level: "Expert",
      progress: 92,
      description: "CNNs, image recognition, object detection, GANs.",
      tags: ["Vision", "Deep Learning", "TensorFlow"],
      difficulty: "Hard",
      image: "https://images.pexels.com/photos/8386422/pexels-photo-8386422.jpeg?auto=compress&w=400",
      estimatedTime: "4 hours",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {/* Sidebar */}
      <div className={`fixed lg:static inset-y-0 left-0 w-64 bg-white border-r transform transition-all duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left ${
                  activeTab === item.id ? "bg-violet-100 text-violet-700" : "hover:bg-gray-100"
                }`}
              >
                <Icon className="w-5 h-5" />
                {item.label}
        ...
        <div className="mt-auto p-4 border-t">
          ...
          <button
            onClick={onLogout}
            className="mt-3 text-red-600 hover:underline flex items-center gap-2 text-sm"
          >
      ...
      <div className="flex-1">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold">
                Good morning, <span className="text-violet-600">{user?.name || "SkillMaster_42"}</span>!
              </h1>
              <p className="text-sm text-gray-600">You're on a 14-day learning streak. Keep it up! 🔥</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative hidden md:block">
              ...
              <input
                type="text"
                placeholder="Search skills..."
                className="pl-10 px-3 py-2 border rounded-lg"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
            </div>
            ...
            >
              <Brain className="w-4 h-4 inline" /> Generate Exam
            </button>
          </div>
        </header>
        ...
        <main className="p-6 space-y-8">
            ...
            {stats.map((stat, index) => {
              ...
              return (
                <div key={index} className="bg-white p-5 rounded-xl border shadow-sm">
                  ...
                  <p className="mt-2 text-gray-700 text-sm font-medium">{stat.label}</p>
                </div>
              );
            })}
          </div>
          ...
          {/* Recommended Skills */}
          <div>
            <h2 className="text-lg font-bold mb-2">Recommended Skills</h2>
            <p className="text-sm text-gray-600 mb-4">Based on your learning history</p>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {recommendedSkills.map((skill, index) => (
                <div key={index} className="bg-white rounded-xl shadow border overflow-hidden hover:shadow-xl transition">
                  
                  <div className="relative h-40">
                    <img src={skill.image} className="w-full h-full object-cover opacity-70" />
                  ...
                  <div className="p-4">
                    ...
                    <button
                      onClick={() => onNavigate("exam-generator")}
                      className="bg-violet-600 hover:bg-violet-700 text-white w-full py-2 rounded-lg"
                    >
                      Generate Exam
                    </button>
                  </div>
          ...
          <section className="bg-white rounded-xl border p-6 shadow-sm">
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                <p className="text-sm text-gray-600">Your latest exam results</p>
              </div>
              ...
              <div className="space-y-3">
                {recentActivity.map((r, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <span className={`px-2 py-1 text-xs rounded-full ${getBadgeClass(r.badge)}`}>
                        {r.badge}
                      </span>

                      <div className="text-right">
                        <p className={`font-bold ${getScoreColor(r.score)}`}>{r.score}</p>
